...
public class ExcelUpdater {
    ...
    public ExcelUpdater(Path excelFile, ExcelMergeOptions options) {
        this.excelFile = excelFile;
        this.options = options == null ? ExcelMergeOptions.withoutHighlight() : options;
    }

    public MergeResult update(Map<String, CompanyData> companiesByOrgNumber) {
        ...
        try (InputStream input = Files.newInputStream(excelFile);
            ...
            for (Row row : sheet) {
                ...
                CompanyData company = companiesByOrgNumber.get(orgNumber);
                if (company == null) {
                    skippedRows++;
